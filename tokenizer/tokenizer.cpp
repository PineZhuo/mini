        ...
        while (true) {
            ...
            switch (current_state) {
                ...
                case INITIAL_STATE: {
                    ...
                    else if (!miniplc0::isprint(ch)) // 非法字符，control codes and backspace
                        invalid = true;
...
//                        pos = previousPos();
                    ...
                    if(miniplc0::isdigit(ch)){
                        ...
                        // 有点问题啊，这里不是应该直接报错？？例如：123abc
                        // 问题解答，在标识符里读完再报错，报错类型是：错误的标识符
                    else if(miniplc0::isalpha(ch)){
                        ss << ch;
...
//                        return std::make_pair(std::make_optional<Token>(TokenType::UNSIGNED_INTEGER, dig, pos, currentPos()), std::optional<CompilationError>());
                ...
                case IDENTIFIER_STATE: {/////标识符
                    // 请填空：
                    // 如果当前已经读到了文件尾，则解析已经读到的字符串
                    // 如果解析结果是关键字，那么返回对应关键字的token，否则返回标识符的token,这里用isKeyToken函数的返回值
                    if(!current_char.has_value()){
//                        unreadLast();
                        std::string str;
//                        ss >> str;
                        str = ss.str();
//                        pos = previousPos();
                        return std::make_pair(std::make_optional<Token>(isKeyToken(str), str, pos, currentPos()), std::optional<CompilationError>());
                    }
                    auto  ch = current_char.value();
                    // 如果读到的字符是数字或字母，则存储读到的字符
...
//                        pos = previousPos();
                ...
                case PLUS_SIGN_STATE: {
                    ...
                    // 因为上一个字符才是加号，并且得到了加号的状态，这一个字符就不是了，需要回退
    ...
    TokenType Tokenizer::isKeyToken(std::string str){
        if(str == "begin")
    ...
    std::string Tokenizer::delFrontZero(std::string str){
        int len = str.length();
        if(len == 1)
            return str;
        int i;
    ...
    bool Tokenizer::isOverflow(std::string str){
