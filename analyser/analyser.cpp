...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseConstantDeclaration() {
		...
		while (true) {
			...
			if (!next.has_value())
				return {};
			// 如果是 const 那么说明应该推导 <常量声明> 否则回退，然后返回
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
        ...
        while(true){
            ...
            if(next.value().GetType() == TokenType::EQUAL_SIGN){
                ...
                _instructions.emplace_back(Operation::STO, getIndex(next1.value().GetValueString()));
            } else{
                unreadToken();
		...
		while (true) {
			...
			switch (next.value().GetType()) {
			    ...
			    case TokenType ::IDENTIFIER:{
			        err = analyseAssignmentStatement();//赋值语句
			        if(err.has_value())
			            return err;
			        break;
			    }
	...
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
		...
		if(type == TokenType::UNSIGNED_INTEGER){
		    std::stringstream ss;
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
		...
		if(!next.has_value())
		    return {};
		if(next.value().GetType() == TokenType::IDENTIFIER && isDeclared(next.value().GetValueString())){//如果已经被声明
		    if(isConstant(next.value().GetValueString())){//如果是常量
		        return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrAssignToConstant);
		    }
		    if(isUninitializedVariable(next.value().GetValueString())){//如果之前没有被赋值
		        varInitialized(next.value());//换符号表
		    }
		    next = nextToken();
		    //'='
		    if(next.value().GetType() != TokenType::EQUAL_SIGN)
		...
		_instructions.emplace_back(Operation::STO, getIndex(name));
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseFactor() {
		...
		switch (next.value().GetType()) {
            case TokenType ::IDENTIFIER :{
                ...
                int32_t off = getIndex(next.value().GetValueString());//标识符在栈上的偏移
                _instructions.emplace_back(Operation::LOD, off);//把偏移为off的数值复制到栈顶
                break;
            }
	...
	//常量，找到了返回true，意味着命名重复了
	bool Analyser::isConstant(const std::string& s) {
		return _consts.find(s) != _consts.end();
	}
    //从未赋初值的符号表中删除，加入赋初值的符号表中
	void Analyser::varInitialized(const Token& tk){
	    auto name = tk.GetValueString();
        _vars[name] = getIndex(name);
        _uninitialized_vars.erase(name);
	}
}